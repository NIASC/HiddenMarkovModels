...
viruses_sequences <- subset(data, data[,7] <= 1e-5)
...
viruses_sequences = viruses_sequences[order(viruses_sequences[,1], viruses_sequences[,8]), ]
...
sequences = c()
VirusesFamily = c()
fileLocation = "/media/StorageOne/zurbzh/annotationFiles_2014/"
...
for (i in 1:length(viruses[,1])) {
  ...
  if (length(splitFamilies) > 1 ) {
    for (j in 1:length(splitFamilies)) {
    	family = unlist(strsplit(splitFamilies[1], split = " "))
   	 if (identical(family[1], "None")) {
     	 splitFamilies = splitFamilies[splitFamilies != splitFamilies[j]] 
   	 }
    }
  }
...
output = data.frame(sequences, VirusesFamily, viruses[,5], viruses[,8])
